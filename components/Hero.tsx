...
export default function Hero({ onEnterApp }: HeroProps) {
  ...
  // Scroll past hero triggers panel transition
  useEffect(() => {
    ...
    >
      ...
      <div className="flex flex-col px-6 md:px-[52px] py-16 md:py-[60px] border-b md:border-b-0 md:border-r border-[#e0ddd7]">
        <div className="max-w-[470px] ml-auto w-full flex-1 flex flex-col justify-center">
          ...
          {/* Divider */}
          <div className="w-10 h-px bg-[#ddd] my-6 anim-fade-in" style={{ animationDelay: '200ms' }} />
      ...
      <div className="hidden md:flex flex-col justify-between bg-[#f0ece4] relative overflow-hidden px-10 py-[60px]">
        ...
        <nav className="flex flex-col gap-[14px] anim-fade-in" style={{ animationDelay: '400ms' }}>
          {['About', 'Experience', 'Work', 'Skills', 'Contact'].map((item) => (
            <button
              ...
              className="text-left text-[9px] tracking-[3px] uppercase text-[#bbb] hover:text-[#666] transition-colors duration-150"
            >
              {item}
            </button>
          ))}
        ...
        {/* Bottom meta */}
        <p className="text-[9px] tracking-[2px] uppercase text-[#ccc] anim-fade-in" style={{ animationDelay: '600ms' }}>
          Portfolio
          <br />
          2026
        </p>
