...
export default function CommandPalette({ open, onClose, onSelect }: CommandPaletteProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-start justify-center pt-[18vh] bg-[#111]/50 dark:bg-[#000]/60 backdrop-blur-[2px]"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[480px] mx-4 bg-[#fafaf8] dark:bg-[#191917] border border-[#e0ddd7] dark:border-[#2c2a27]"
        onClick={(e) => e.stopPropagation()}
        onKeyDown={handleKeyDown}
      >
        {/* Input row */}
        <div className="flex items-center gap-3 px-4 py-[11px] border-b border-[#e0ddd7] dark:border-[#2c2a27]">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-[#ccc] dark:text-[#3c3a37] flex-shrink-0">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[13px] text-[#111] dark:text-[#e5e2db] placeholder-[#ccc] dark:placeholder-[#3c3a37] outline-none"
          />
          <kbd className="text-[8px] tracking-[1px] uppercase text-[#ccc] dark:text-[#3c3a37] border border-[#e0ddd7] dark:border-[#2c2a27] px-[6px] py-[2px] font-sans">
            ESC
          </kbd>
        </div>
        ...
        <div className="py-[4px]">
          {filtered.length === 0 && (
            <div className="px-4 py-3 text-[11px] text-[#aaa] dark:text-[#484542]">No results</div>
          )}
          {filtered.map((item, i) => (
            <button
              key={item.id}
              onClick={() => { onSelect(item.id); onClose() }}
              className={`w-full text-left px-4 py-[10px] flex items-center justify-between transition-colors duration-75 ${
                i === highlighted
            ...
            >
              <div>
                ...
                <div className="text-[10px] text-[#aaa] dark:text-[#484542]">{item.description}</div>
              </div>
              {i === highlighted && (
